...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{lidar_and_kinect}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.9\textwidth]{sensor_connections}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\textwidth]{mobile_platform_files}
...
The first velocity command publisher was implemented as a local keyboard listener. Keystrokes are  registered by \texttt{getchar()} in a continuous while loop. The terminal behavior has been altered through a call to \texttt{system(''/bin/stty raw'')}, which will pass each keystroke directly to \texttt{getchar()}, without requiring the user to press \keystroke{Enter}. This will rented \keystroke{Ctrl}$+$\keystroke{C} useless, as the combination will be split into two inputs. A distilled keyboard listener loop is listed below:
...
After a user has entered \keystroke{Ecs}, the program can be exited as normal with \keystroke{Ctrl}$+$\keystroke{C}.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=1\textwidth]{conn_diag_motors}
